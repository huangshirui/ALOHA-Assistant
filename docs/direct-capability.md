...
M2 proves that the selected Runtime Backend can call one ALOHA-owned Capability without receiving broad ALOHA authority:

```text
PWA
  -> Gateway
  -> Agent Control
       |- selects capabilities allowed for the current Run
...
The important result is not merely that arithmetic works. The result is that **Agent Control decides what the Runtime may call, the Runtime receives only that allowed capability, and invocation is rejected without the grant for that Run**.
...
This capability is deliberately authority-free. M2 therefore does **not** invent a fake Principal（权限主体） or pretend that trusted LifeSpace Identity / Grant integration already exists. Capabilities that require user/domain authority remain gated on the later trusted Identity / Authorization slice.
...
Agent Control computes the current allowed set from the authority/context available for the Run. In M2 the trusted authority context is intentionally empty, so only capabilities with:
...
If `CAPABILITY_GRANT_SIGNING_KEY` is not configured, Agent Control exposes **zero** runtime capabilities. Missing deployment configuration therefore fails closed rather than silently granting authority.
...
The route is public only because the external n8n Runtime must be able to reach it. Authority remains in the signed grant and Agent Control verification. The public Gateway remains transport-only.
...
4. the Runtime request contains `math.calculate` and no broader capability set than Agent Control admitted;
...
- a n8n Workflow Capability（工作流能力） — that is the next separate MVP slice.
